...
def waymo_local_collate_fn(batch, time_steps=10, current_step=3, sampling_time=0.5, GD=16, GS=1400, is_local=True, halfwidth=100): # GS = max number of static roadgraph element (1400), GD = max number of dynamic roadgraph (16)
    ...
    for data in batch:
        ...
        states_padding_mask = states_padding_mask[:,::sampling_freq][:,:time_steps]
        states_feat[:,:,-1] = np.where(states_padding_mask, states_feat[:,:,-1], states_feat[:,:,-1]/1e6)

        # if is_local:
        #     center_x, center_y = states_feat[sdc_mask][0,current_step,:2]
            
        #     # make global coordinate to local centered to sdv and normalize 
        #     # Also, pad features outside width. Also, update padding mask 
        #     states_feat[:,:,:2] = np.where(np.tile(states_padding_mask[:,:,None], (1,1,2)), states_feat[:,:,:2], states_feat[:,:,:2]-np.array([center_x,center_y]))
        ...
        #     traffic_light_feat[traffic_light_padding] = -1
        ...
        if is_local:
            ...
            # make global coordinate to local centered to sdv and normalize 
            # Also, pad features outside width. Also, update padding mask 
            states_feat[:,:,:2] = np.where(np.tile(states_padding_mask[:,:,None], (1,1,2)), states_feat[:,:,:2], (states_feat[:,:,:2]-np.array([center_x,center_y]))/halfwidth)
