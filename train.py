...
def main(cfg):
    ...
    dloader_train = DataLoader(dataset_train, batch_size=cfg.dataset.train.batchsize, collate_fn=waymo_collate_fn, num_workers=8)
    dloader_valid = DataLoader(dataset_valid, batch_size=cfg.dataset.valid.batchsize, collate_fn=waymo_collate_fn, num_workers=8)
    ...
    trainer = pl.Trainer(max_epochs=cfg.max_epochs, gpus=cfg.device_num, gradient_clip_val=5,accelerator='ddp',val_check_interval=4000, 
                                limit_train_batches=1.0, limit_val_batches=0.4, log_every_n_steps=1000)
